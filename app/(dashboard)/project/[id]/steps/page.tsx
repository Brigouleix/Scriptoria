...
const STEPS_META = [
  {
    ...
    doneBorder: 'border-amber-500/40',
    doneDot: 'bg-amber-500',
  },
  {
    number: 2,
  ...
  },
  {
    ...
    doneBorder: 'border-violet-500/40',
    doneDot: 'bg-violet-500',
  },
] as const
  ...
  return (
    <div className="flex flex-col gap-8 max-w-2xl">
      ...
      <div className="flex items-center gap-2 text-[var(--text-muted)] text-sm flex-wrap">
        <Link href="/dashboard" className="hover:text-[var(--text-primary)] transition-colors">{tp('breadcrumb')}</Link>
        <ChevronRight size={14} />
        <Link href={`/project/${id}`} className="hover:text-[var(--text-primary)] transition-colors">{project.title}</Link>
        <ChevronRight size={14} />
        <span className="text-[var(--text-secondary)]">Méthode Snowflake</span>
      </div>

      {/* Header */}
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">{project.title}</h1>
        {project.genre && <p className="text-[var(--text-muted)] text-sm">{project.genre}</p>}
        <p className="text-[var(--text-muted)] text-sm mt-1">{completedCount} / {STEPS_META.length} étapes complétées</p>
      </div>
        ...
        {STEPS_META.map((step) => {
          ...
          return (
            <Link
              ...
              className={`group border rounded-2xl p-6 flex flex-col gap-3 transition-all duration-200 bg-[var(--bg-card)]
                hover:scale-[1.01] hover:shadow-lg
                ${isDone ? step.doneBorder : 'border-[var(--border)]'}
              `}
            >
              {/* Step number + status */}
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${isDone ? step.doneDot : 'bg-[var(--text-muted)]'}`} />
                ...
                {isDone
                  ...
                  : <Circle size={18} className="text-[var(--text-muted)] group-hover:text-[var(--text-secondary)] transition-colors" />
                }
              </div>

              {/* Title */}
              <h2 className="text-lg font-bold text-[var(--text-primary)]">{t(step.labelKey)}</h2>
              ...
              {preview ? (
                ...
                <p className="text-[var(--text-muted)] text-sm leading-relaxed">{t(step.descKey)}</p>
              )}

              {/* CTA */}
              <div className="flex items-center gap-1.5 text-xs font-medium mt-1 text-[var(--text-muted)] group-hover:text-[var(--accent)] transition-colors">
